...
12. [付録: 年次更新の流れまとめ](#付録-年次更新の流れまとめ)
...
```json
{
  "tier1": [{ "name": "株式会社ワイエムジー", "amount": "50万" }],
  "tier2": [
    { "name": "株式会社新来島豊橋造船", "amount": "10万" },
    { "name": "本多電子株式会社", "amount": "10万" }
  ],
  "tier3": [{ "name": "株式会社金トビ志賀", "amount": "7万" }]
}
```
...
- `name` がページに表示されます
- `amount` は**表示されません**。HTMLのコメントとして埋め込まれる管理用のメモです
