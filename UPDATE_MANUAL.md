...
9. [開発環境のセットアップとGit運用ルール](#9-開発環境のセットアップとgit運用ルール)
...
## 9. 開発環境のセットアップとGit運用ルール

技科大祭HPリポジトリに、改行コード・空白・インデントの表記ゆれを防ぐための自動整形設定（Prettier / EditorConfig / Husky）を導入しています。

作業を開始する開発者は、以下の手順を必ず実施してください。

### 全員が実施すること

1. **変更内容のコミット**  
   現在の作業内容（変更中のファイル）がある場合は先にコミットしてください。
2. **リポジトリの再クローン**  
   改行コードの設定変更を既存のフォルダに正しく反映させるため、リポジトリを一度削除し、再度クローンしてください。
...
3. **拡張機能のインストール**  
   AntigravityまたはVS Codeで、以下2つの拡張機能をインストールしてください。
   - **Prettier - Code formatter**
   - **EditorConfig for VS Code**

4. **依存パッケージのインストール**  
   以下を実行してください。これにより、コミット時に自動整形する仕組み（Husky）が有効になります。
...
5. **git blame 設定の適用**  
   ローカルの `git blame` 表示から今回の一括整形コミットを除外するため、以下を**一度だけ**実行してください。
...
- **保存時:** 自動でコードが整形されます（開発体験の向上）。
- **コミット時:** 自動整形（Husky + lint-staged）が自動実行されます。
- **PR作成後:** GitHub Actionsによるフォーマットチェックが行われ、失敗するとマージできません。  
  _(エラーが出た場合は `npm run format` を実行してから再度コミット・pushしてください)_
