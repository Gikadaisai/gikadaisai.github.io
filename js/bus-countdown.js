...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  function updateBusCountdown() {
    ...
    if (day === 6) {
      ...
      if (busScheduleNoteEl) {
        busScheduleNoteEl.textContent =
          "※表示は平日ダイヤです。技科大祭当日の土日ダイヤとは異なります。";
        busScheduleNoteEl.style.display = "block";
      }
    }

    var nextBus = schedule.find(function (time) {
    ...
    if (!nextBus) {
      ...
      if (tomorrowDay === 6) {
        tomorrowSchedule = timetable.saturday;
      } else if (tomorrowDay === 0) {
        tomorrowSchedule = timetable.sunday;
      } else {
        tomorrowSchedule = timetable.weekday;
      }
